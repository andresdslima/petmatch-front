...
const validationSchema = Yup.object({
	...
	celular: Yup.number()
		...
		.max(11, '11 dígitos')
		.required('*'),
	cidade: Yup.string().min(3, ' sua cidade').required('*'),
	estado: Yup.string().min(2, 'Escolha seu estado').required('*'),
	cep: Yup.string().min(7, '8 dígitos').max(8, '8 dígitos').required('*'),
	endereco: Yup.string().min(6, 'Endereço completo').required('*'),
	enderecoNumero: Yup.number().max(5, 'Máximo 5 números').required('*'),
			...
			<Form onSubmit={formik.handleSubmit}>
				<Styled.ContainerForm>
					...
					<Styled.InputContainer>
						...
						{formik.errors.endereco && <small>{formik.errors.endereco}</small>}
						<Styled.SInput
							type="text"
							name="endereco"
							id="endereco"
				...
				<Styled.ButtonContainer className="my-5">
					<Styled.SButton variant="primary" type="button">
						Voltar
					</Styled.SButton>
